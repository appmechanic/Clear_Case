...
class DisputeInsightsProvider with ChangeNotifier {
  ...
  // MASTER LIST: Holds everything from Firestore
  List<Map<String, dynamic>> _allDisputes = [];
  // DISPLAY LIST: What the UI shows
  ...
  Future<void> fetchDisputes(String caseId) async {
    ...
    try {
      ...
      // Fetch log counts in parallel
      await Future.wait(tempDisputes.map((dispute) async {
        final logSnap = await _db.collection('users').doc(userId).collection('cases')
            .doc(caseId).collection('disputeRecords').doc(dispute['id'])
  ...
  void _calculateStats(Map<String, dynamic> data) {
    final category = data['category'] ?? "";
    final status = data['disputeStatus'] ?? "Open";
    if (category == "Communication") commCount++;
    else if (category == "Transfer Issues") transferCount++;
    else if (category == "Payment Disputes") paymentCount++;
  ...
  void _resetStats() {
    commCount = 0; transferCount = 0; paymentCount = 0; openCount = 0; resolvedCount = 0;
  }

  Stream<DocumentSnapshot> getDisputeStream(String caseId, String disputeId) {
    return _db.collection('users').doc(_auth.currentUser?.uid)
        .collection('cases').doc(caseId).collection('disputeRecords').doc(disputeId).snapshots();
  }

  Stream<List<Map<String, dynamic>>> getDisputeLogs(String caseId, String disputeId) {
    return _db.collection('users').doc(_auth.currentUser?.uid)
        .collection('cases').doc(caseId).collection('disputeRecords').doc(disputeId)
        .collection('logs').orderBy('createdAt', descending: true).snapshots()
        .map((snap) => snap.docs.map((doc) => {...doc.data(), 'id': doc.id}).toList());
  }

  Future<void> saveLog({
    required String caseId, required String disputeId, String? logId,
    ...
    final docRef = _db.collection('users').doc(userId).collection('cases').doc(caseId)
        .collection('disputeRecords').doc(disputeId).collection('logs');
    if (logId == null) {
  ...
  Future<void> deleteLogWithStorage(String caseId, String disputeId, Map<String, dynamic> log) async {
    ...
    await _db.collection('users').doc(_auth.currentUser?.uid).collection('cases').doc(caseId)
        .collection('disputeRecords').doc(disputeId).collection('logs').doc(log['id']).delete();
  }
}
